...
void FileMgmt::parseCsvFileValues() {
  ...
  for (int i = 0; i < this->content.size(); i++) {
    if (std::regex_match(this->content[i],
                         std::regex("[+-]?([0-9]*[.])?[0-9]+"))) {
...
void FileMgmt::writeOutputValuesIntoFile() {
  std::vector<std::string> header = {
      "primeira_entrada", "primeira_saida",    "ultima_entrada", "ultima_saida",
      "dia_mes",          "horas_trabalhadas", "horas_extras"};
    ...
    if (std::regex_match(this->content[i],
      ...
      if (counter == 4) {
        outputFile << std::fixed << std::setprecision(2)
                   << this->outputResult[i].first << ", ";
        outputFile << std::fixed << std::setprecision(2)
                   << this->outputResult[i].second << "\n";
        counter = 0;
      }
    }
  }
